...
`scripts/legacy/migrate.py` and `scripts/legacy/fix_db.py` are deprecated, pre-Alembic one-off
scripts — see their docstrings. Don't add new ones like them; use a real migration.

## Operational scripts

These are maintenance/setup scripts, not part of the running app (nothing in `app/`
imports them). Run them as modules from the repo root (for example,
`python -m scripts.seed.seed_academia`), against whichever
...
**Admin/maintenance**
- `python -m scripts.admin.make_admin <username> grant|revoke` — grant or revoke admin access for a user.
  Requires typing the username again to confirm; logs every change to `AdminAuditLog`.
- `python -m scripts.audit.check_materials` — inspects the `Material` table.
- `python -m scripts.audit.audit_broken_material_links [--apply]` — finds (and, with `--apply`, deactivates)
  `Material` rows whose `file_url` points at a local static file that doesn't exist on
  disk. Defaults to a dry run.
- `python -m scripts.maintenance.cleanup_unembeddable_materials [--dry-run]` — checks external-URL materials for
...
- `scripts/seed/seed_materials.py`, `scripts/seed/seed_30_courses.py` — near-duplicate legacy material seeders;
  `seed_materials.py` is the more complete/current one.
- `scripts/seed/seed_100_level_science_500.py`, `scripts/seed/seed_200_to_400_level_science.py`,
  `scripts/seed/seed_200_to_400_COMPLETE.py` — Faculty of Science material seeders;
...
under `app/static/materials/` first.
