...
def generate_topic_explanation(course_code, course_title, topic_title, video=None):
    ...
    system_prompt = (
        ...
        "appropriate for a student encountering it for the first time -- something a "
        "student can actually study from, not a wall of AI-generated text. Write like a "
        ...
        + "\n\nStructure the output as clean HTML, using only the sections that genuinely "
        "apply to this specific topic (skip any that would just be filler for it):\n"
        "- <h3>What it means</h3> -- one or two <p> paragraphs explaining the topic itself "
        "and why it matters in this course.\n"
        "- <h3>Key concepts</h3> -- a <ul> of 3-6 <li> bullets, each a specific concept "
        "within this topic (not vague restatements of the topic name).\n"
        "- <h3>Terminology</h3> -- a <ul> of the important terms a student must know, each "
        "as <li><strong>Term</strong> — one-line definition</li>. Omit this section entirely "
        "if the topic doesn't introduce specific new vocabulary.\n"
        "- <h3>Example</h3> -- one concrete worked example. Use a <pre><code> block for a "
        "code/formula example if the topic is technical; otherwise a short worked scenario "
        "in prose.\n"
        "- <h3>For your exam</h3> -- a <ul> of 2-4 bullets on what's typically expected: "
        "specific things to be able to explain, calculate, or apply, not generic study advice.\n"
        "- <h3>Common mistakes</h3> -- only include this section if there's a genuine, "
        "well-known misconception or error students make on this specific topic; omit it "
        "entirely rather than inventing a generic one.\n"
        "<strong> for emphasis, <code> for inline technical terms. No Markdown, no "
        "<html>/<body> wrapper, no surrounding code fences around the whole response. Aim "
        "for 300-550 words total -- thorough enough to actually study from, still focused."
    )
